...
public class Collider {
		...
		public boolean[] intersects(Collider[] colliderList) {
			...
			for (int i = 0; i < colliderList.length; i++) {
				...
				Line[] otherLines = collider.getLinesRect();
				Line[] lines = this.getLinesCross();
				for (int j = 0; j < 4; j++) {
...
//			marker.rect((float) getX(), (float) getY(), (float) width, (float) height);
		...
		public Line[] getLinesCross() {
			Line[] lines = new Line[4];
			lines[0] = new Line(getCenterX(), y, getCenterX(), y + height/2); // vertical middle
			lines[1] = new Line(x + width/2, getCenterY(), x + width, getCenterY()); // horiz middle
			lines[2] = new Line(getCenterX(), y + height/2, getCenterX(), y + height); // vertical middle
			lines[3] = new Line(x, getCenterY(), x + width/2, getCenterY()); // horiz middle
			return lines;
		}
